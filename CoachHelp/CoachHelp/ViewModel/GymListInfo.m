...
#define BrandAPI @"/api/v2/coaches/%ld/staffs/brands/%ld/shops/"
...
-(void)requestWithBrand:(Brand *)brand result:(void (^)(BOOL, NSString *))result
{
    ...
    [MOAFHelp AFGetHost:ROOT bindPath:[NSString stringWithFormat:BrandAPI,CoachId,(long)brand.brandId] param:nil success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
...
-(NSArray *)getHavePowerGymsWithGyms:(NSArray *)gyms
{
    ...
    for (Gym *gym in gyms) {
        ...
        for (Gym *localGym in self.gyms) {
            ...
            if (gym.shopId == localGym.shopId && gym.brand.brandId == localGym.brand.brandId && gym.havePermission) {
