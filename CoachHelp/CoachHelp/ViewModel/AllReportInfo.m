...
#define API @"/api/v2/coaches/%ld/reports/%@/glance/"
...
-(void)requestInfoWithGym:(Gym *)gym result:(void (^)(BOOL, NSString *))result
{
    ...
    NSString *api = [NSString stringWithFormat:API,CoachId,typeStr];
...
-(ReportInfo *)getShowInfoWithGym:(Gym *)gym
{
    ...
    }else
    {
        ...
        for (ReportInfo *info in self.reportInfos) {
            
            if ([info.gym.name isEqualToString:gym.name]) {
