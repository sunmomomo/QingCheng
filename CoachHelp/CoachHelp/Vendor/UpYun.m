...
- (void)uploadSavekey:(NSString *)savekey data:(NSData*)data filePath:(NSString*)filePath {
    ...
    if (filePath) {
        ...
        fileSize = (unsigned long)[fileDictionary fileSize];
    }
...
- (void)uploadLivePhoto:(PHLivePhoto *)livePhotoAsset saveKey:(NSString *)saveKey extParams:(NSDictionary *)extParams {
    ...
    NSUInteger photoFileSize = (unsigned long)[[NSFileManager defaultManager] attributesOfItemAtPath:PATH_PHOTO_FILE error:nil].fileSize;
    NSUInteger moveFileSize = (unsigned long)[[NSFileManager defaultManager] attributesOfItemAtPath:PATH_MOVIE_FILE error:nil].fileSize;
...
+(NSString * )getSaveKey {
    ...
    NSNumber *number = [NSNumber numberWithUnsignedInteger:[NSString stringWithFormat:@"%ld%ld%.0f",(long)[self getYear:d],(long)[self getMonth:d],[[NSDate date] timeIntervalSince1970]].hash];
    
    NSString *str = [NSString stringWithFormat:@"/%@.jpg",[number stringValue]];
