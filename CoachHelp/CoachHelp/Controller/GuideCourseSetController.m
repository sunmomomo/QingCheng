...
@interface GuideCourseSetController ()<UITextFieldDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate>
...
@implementation GuideCourseSetController
...
-(void)reloadData
{
    ...
    if (self.course.imgUrl.absoluteString) {
        ...
        if ([self.course.imgUrl.absoluteString rangeOfString:@"!"].length) {
            ...
            }else if ([self.course.imgUrl.absoluteString rangeOfString:@"/watermark/"].length){
...
-(void)confirmClick
{
    ...
    GuideCoursePlanController *svc = [[GuideCoursePlanController alloc]init];
