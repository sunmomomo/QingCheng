...
-(void)createUI
{
    ...
    if (!self.isAdd) {
        ...
        if (self.course.gyms.count >1) {
            ...
            hintLabel.text = @"请在健身房管理App中管理课程种类";
    ...
    if (!self.isAdd) {
        ...
        suitHintLabel.text = @"请在健身房管理App中管理适用场馆";
        ...
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"打开APP"];
...
-(void)suitChoose
{
    ...
    [self showAppAlertWithTitle:@"修改适用场馆" andSubtitle:nil];
...
-(BOOL)checkNext
{
    ...
    if (self.course.type == CourseTypeGroup) {
        ...
        if (!self.capacityTF.text.length) {
            ...
            [self errorWithMessage:@"课程可约人数尚未填写"];
