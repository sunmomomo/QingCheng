...
-(void)createUI
{
    ...
    self.topView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, MSW, self.filter.infoType == ReportInfoTypeSell?Height320(40)*6:Height320(40)*4)];
    ...
    self.secondTF.placeholder = self.filter.infoType == ReportInfoTypeSchedule?@"会员":self.filter.infoType == ReportInfoTypeSell?@"交易类型":@"状态";
    
    self.secondTF.type = QCTextFieldTypeCell;
    
    if (self.filter.infoType == ReportInfoTypeSchedule) {
        
        self.secondTF.placeholder = @"会员";
        
        self.secondTF.noLine = YES;
        
        if (self.filter.student) {
            
            self.secondTF.text = self.filter.student.name;
            
        }else{
            
            self.secondTF.text = @"所有会员";
    ...
    if (self.filter.infoType == ReportInfoTypeSell) {
        ...
        if (self.filter.payWay) {
            ...
            self.thirdTF.text = self.filter.payWay == PayWayQRCode?@"微信扫码支付":self.filter.payWay == PayWayWeChat?@"微信支付":self.filter.payWay == PayWayCash?@"现金支付":self.filter.payWay == PayWayCard?@"刷卡支付":self.filter.payWay == PayWayGive?@"赠送":self.filter.payWay == PayWayTransfer?@"转账支付":@"其他支付";
        ...
        self.forthTF.placeholder = @"会员";
        
        self.forthTF.type = QCTextFieldTypeCell;
        
        if (self.filter.student) {
            
            self.forthTF.text = self.filter.student.name;
            
        }else{
            
            self.forthTF.text = @"所有会员";
    ...
    if (self.filter.infoType == ReportInfoTypeSchedule) {
        ...
        self.secondTF.inputView = userKV;
    ...
    }else if(keyboadeView.tag == 202)
    {
        ...
        if (self.filter.infoType == ReportInfoTypeSchedule) {
            ...
            if (keyboadeView.tag == 101)
            {
                ...
                self.secondTF.text = self.userPV.titleArray[self.userPV.currentRow];
            ...
            }else if(keyboadeView.tag == 102)
            {
                ...
                self.forthTF.text = self.userPV.titleArray[self.userPV.currentRow];
