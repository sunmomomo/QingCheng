...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        self.frame = CGRectMake(MSW/2-Width(88), MSH/2-Height(93), Width(176), Height(186));
...
- (void)configOwnViews
{
    _imageTip.image = [UIImage imageNamed:@"record_animate_1"];
...
{    
    _imageTip.frame = CGRectMake(Width(35), Height(28), Width(107), Height(98));
    
    [_countdownTip sameWith:_imageTip];
    
    _tip.frame = CGRectMake(Width(15), _imageTip.bottom+Height(18), self.width-Width(30), Height(24));
}
...
- (void)onRecorderPeakChanged:(ChatSoundRecorder *)rec
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString *tip = [NSString stringWithFormat:@"record_animate_%d", (int)peaker];
...
- (void)showRecordView
{
    UIViewController *topViewController = ((UINavigationController*)[MOAppDelegate getCurrentVC]).topViewController;
...
- (void)startRecording
{
    ...
    if (![self initRecord])
    {
        [[[UIAlertView alloc]initWithTitle:@"初始化录音机失败" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil]show];
        return;
    }
...
- (void)stopRecord
{
    ...
    else
    {
        ...
        [self.delegate sendVoiceWithSound:audioData duration:dur];
