...
-(void)requestSystemResult:(RequestCallBack)result
{
    ...
    NSString *str = @"{\"gym\":[3,4,5,6,7,10,14,16],\"system\":[20001],\"study\":[10001,10002,10003,10004,10005,10006,10007,10008],\"reply\":[20003]}";
        ...
        if (userSig.length) {
            ...
            [[TIMManager sharedInstance] login: login_param succ:^(){
                ...
                [userInfo requestResult:^(BOOL success, NSString *error) {
                    ...
                    [[TIMFriendshipManager sharedInstance]SetNickname:userInfo.user.username succ:^{
                        
                        [[TIMFriendshipManager sharedInstance]SetFaceURL:userInfo.user.iconURL.absoluteString succ:^{
...
-(void)createChatWithUsers:(NSArray *)users result:(UsersCallBack)result
{
    ...
    if (allUsers.count == 1) {
        ...
        [info requestResult:^(BOOL success, NSString *error) {
            ...
            [allUsers insertObject:info.user atIndex:0];
...
-(void)readAllResult:(RequestCallBack)result
{
    ...
    [MOAFHelp AFPutHost:ROOT bindPath:ClearAPI putParam:para.data success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
