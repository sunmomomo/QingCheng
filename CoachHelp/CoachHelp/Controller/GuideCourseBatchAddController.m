//
//  GuideCourseBatchAddController.m
//  CoachHelp
//
...
@interface GuideCourseBatchAddController ()<UITextFieldDelegate,QCKeyboardViewDelegate>
...
@implementation GuideCourseBatchAddController
...
-(void)createUI
{
    ...
    UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MSW, self.course.type == CourseTypePrivate?Height320(40)*2:Height320(40))];
...
-(void)keyboardConfirm:(QCKeyboardView *)keyboardView
{
    ...
    if (keyboardView.tag == 101) {
        ...
        if (self.endTF) {
            ...
            self.endTF.text = [NSString stringWithFormat:[self.startOP.hour integerValue]+1>=10?@"%ld:%@":@"0%ld:%@",(long)[self.startOP.hour integerValue]+1,self.startOP.minute];
...
-(void)check
{
    ...
    self.hintLabel.text = weekStr.length?self.course.type == CourseTypeGroup?[NSString stringWithFormat:@"每个%@ %@有此课程",weekStr,self.startTF.text]:[NSString stringWithFormat:@"每个%@ %@-%@有此课程",weekStr,self.startTF.text,self.endTF.text]:@"";
