...
- (void)__initView {
    ...
    UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MSW, PTXTOOLBAR_HEIGHT)];
...
//    UIBarButtonItem *blankButton = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil];
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(MSW - 85, 0, 85, PTXTOOLBAR_HEIGHT)];
    ...
    [button addTarget:self action:@selector(completion) forControlEvents:UIControlEventTouchUpInside];
    
    [topView addSubview:button];
    
//    UIBarButtonItem *completionButton = [[UIBarButtonItem alloc]initWithCustomView:button];
...
//    [toolbar setItems:@[blankButton,completionButton]];
//    [self addSubview:toolbar];
    ...
    datePickerView.backgroundColor = RGB_YF(244, 244, 244);
...
//            [datePickerView selectRow:selectedHourRow inComponent:3 animated:NO];
            break;
            case PTXDatePickerViewShowModelMonthDay:
        {
            totalTitleLabel = 2;
            titleArray = @[@"月",@"日"];
        }
    ...
    for (NSInteger i = 0; i < titleArray.count; i++) {
        ...
        titleLabel.backgroundColor = RGB_YF(244, 244, 244);
...
- (void)__initData {
    ...
    for (NSInteger i = _minYear; i <= currentYear; i ++) {
        [yearArray addObject:[NSString stringWithFormat:@"%@",@(i)]];
    }
    selectedYearRow = [yearArray indexOfObject:[NSString stringWithFormat:@"%@",@(currentYear)]];
    
    //初始化月份数组(1-12)。
    monthArray = [[NSMutableArray alloc]init];
    for (NSInteger i = 1; i <= currentMonth; i++) {
        [monthArray addObject:[NSString stringWithFormat:@"%@",@(i)]];
    }
    ...
    //默认 是 31 天，currentday 是当前天数
    for (NSInteger i = 1; i <= 31; i++) {
        [dayArray addObject:[NSString stringWithFormat:@"%@",@(i)]];
    }
    ...
    for (NSInteger i = 0; i <= currentHour; i++) {
        [hourArray addObject:[NSString stringWithFormat:@"%@",@(i)]];
    }
    ...
    for (NSInteger i = 0; i <= currentMinute; i++) {
        [minuteArray addObject:[NSString stringWithFormat:@"%@",@(i)]];
    }
...
- (void)showViewWithDate:(NSDate *)date animation:(BOOL)animation{
    ...
    if (self.datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay) {
        year = 2016;
    }
    ...
    if (self.datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay)
    {
        ...
        [datePickerView selectRow:[monthArray indexOfObject:[NSString stringWithFormat:@"%@",@(month)]] inComponent:0 animated:YES];
        
        [datePickerView selectRow:[dayArray indexOfObject:[NSString stringWithFormat:@"%@",@(day)]] inComponent:1 animated:YES];

    }else
    {
        [datePickerView selectRow:[yearArray indexOfObject:[NSString stringWithFormat:@"%@",@(year)]] inComponent:0 animated:YES];
        [datePickerView selectRow:[monthArray indexOfObject:[NSString stringWithFormat:@"%@",@(month)]] inComponent:1 animated:YES];
        [datePickerView selectRow:[dayArray indexOfObject:[NSString stringWithFormat:@"%@",@(day)]] inComponent:2 animated:YES];

    }
    
    
    if (_datePickerViewShowModel == PTXDatePickerViewShowModelDefalut) {
        [datePickerView selectRow:[hourArray indexOfObject:[NSString stringWithFormat:@"%@",@(hour)]] inComponent:3 animated:YES];
        [datePickerView selectRow:[minuteArray indexOfObject:[NSString stringWithFormat:@"%@",@(minute)]] inComponent:4 animated:YES];
    }else if (_datePickerViewShowModel == PTXDatePickerViewShowModelYearMonthDayHour) {
        [datePickerView selectRow:[hourArray indexOfObject:[NSString stringWithFormat:@"%@",@(hour)]] inComponent:3 animated:YES];
    }
//    else if (_datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay)
//    {
//                  
//    }
}
...
- (void)resetDayArrayWithYear:(NSInteger)year month:(NSInteger)month {
    ...
    if (_maxYear == year && maxMonth == month && self.datePickerViewShowModel != PTXDatePickerViewShowModelMonthDay) {
    ...
    if (self.datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay && month == 2) {
        totalDay = 29;
    }
...
- (void)updateCurrentDateArray {
    ...
    if (self.datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay) {
        currentYear = 2016;
        currentMonth = [[monthArray objectAtIndex:selectedMonthRow] integerValue];
        currentDay = [[dayArray objectAtIndex:selectedDayRow] integerValue];
        currentHour = 0;
        
    }else
    {
        currentYear = [[yearArray objectAtIndex:selectedYearRow] integerValue];
        currentMonth = [[monthArray objectAtIndex:selectedMonthRow] integerValue];
        currentDay = [[dayArray objectAtIndex:selectedDayRow] integerValue];
        currentHour = [[hourArray objectAtIndex:selectedHourRow] integerValue];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (_datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay && component > 1) {
        return;
    }
    ...
    switch (component) {
        case 0:
        {
            ...
            if (_datePickerViewDateRangeModel == PTXDatePickerViewDateRangeModelCustom) {
                /**
                 *  如果是自定义模式(即设定了最小和最大年份)，则不用限制月份、天数、小时和分钟，所以不用
                 *  做下面的更新操作。
                 */
                return;
            }
        ...
        case 1:
        {
            ...
            if (_datePickerViewDateRangeModel == PTXDatePickerViewDateRangeModelCustom) {
                /**
                 *  如果是自定义模式(即设定了最小和最大年份)，则不用限制天数、小时和分钟，所以不用
                 *  做下面的更新操作。
                 */
                return;
            }
        ...
        case 2:
        {
            ...
            if (_datePickerViewDateRangeModel == PTXDatePickerViewDateRangeModelCustom) {
                /**
                 *  如果是自定义模式(即设定了最小和最大年份)，则不用限制小时和分钟，所以不用
                 *  做下面的更新操作。
                 */
                return;
            }
        ...
        case 3:
        {
            ...
            if (_datePickerViewDateRangeModel == PTXDatePickerViewDateRangeModelCustom) {
                /**
                 *  如果是自定义模式(即设定了最小和最大年份)，则不用限制分钟，所以不用
                 *  做下面的更新操作。
                 */
                return;
            }
            
            if (_datePickerViewShowModel == PTXDatePickerViewShowModelMonthDay) {
                return;
            }
