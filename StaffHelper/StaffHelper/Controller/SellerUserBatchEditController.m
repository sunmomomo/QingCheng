//
//  SellerUserBatchEditController.m
//  StaffHelper
//
...
@interface SellerUserBatchEditController ()<UITableViewDelegate,MOTableViewDatasource,UserChooseViewDatasource,UITextFieldDelegate,UIAlertViewDelegate,SellerUserDeleteCellDelegate>
...
@implementation SellerUserBatchEditController
{
    UIView *_header;
    UIView *_filterView;
}
...
-(void)reloadData
{
    ...
    {
        [info requestWithSeller:self.seller andGym:self.gym success:^{
            [weakS requestSuccessData:info];
...
-(void)createUI
{
    ...
    {
        self.title = self.seller.type == SellerTypeNormal?@"批量修改":@"批量分配";
    }
    ...
    if (self.isChooseStudent)
    {
    ...
    }else if (self.seller.type == SellerTypeNone) {
        ...
        [distributeButton setTitle:@"分配销售" forState:UIControlStateNormal];
        ...
        [changeButton setTitle:@"变更销售" forState:UIControlStateNormal];
...
-(void)func:(UIButton*)button
{
    ...
    if (button.tag == 1) {
        ...
        SellerChangeController *svc = [[SellerChangeController alloc]init];
        ...
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"确定将所选会员从%@名下移除？",self.seller.name] message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView.tag == 0) {
        ...
        if (buttonIndex == 1) {
            ...
            [info deleteUsers:self.chooseArray withSeller:self.seller withGym:self.gym result:^(BOOL success, NSString *error) {
                ...
                if (success) {
                    ...
                    for (MOViewController *vc in self.navigationController.viewControllers) {
                        ...
                        if ([NSStringFromClass([vc class]) isEqualToString:@"SellerListController"]) {
...
- (YFSearchResultSellerListBatchVC *)resultVC
{
    if (!_resultVC)
    {
        ...
        _resultVC.sellerBatchEditVC = self;
