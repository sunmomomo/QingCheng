...
-(void)naviTitleClick
{
    ...
    if (self.filter.infoType == ReportInfoTypeSchedule) {
        ...
        svc.permission = [Permission courseReportPermission];
        
    }else if(self.filter.infoType == ReportInfoTypeSell){
        
        svc.permission = [Permission sellReportPermission];
        
    }else{
        
        svc.permission = [Permission checkinReportPermission];
    ...
    svc.changed = ^(Gym *gym){
        ...
        weakS.title = weakS.filter.infoType == ReportInfoTypeSchedule?weakS.gym.name.length?weakS.gym.name:@"全部场馆":weakS.gym.name.length?weakS.gym.name:@"全部场馆";
...
-(void)naviRightClick
{
    ...
    ReportExportController *svc = [[ReportExportController alloc]init];
