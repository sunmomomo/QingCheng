...
@interface MeasureEditController ()<QCKeyboardViewDelegate,MONaviDelegate,UITextFieldDelegate,TestPictureViewDelegate,UIAlertViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate>
...
@implementation MeasureEditController
...
-(void)createData
{
    ...
    [self.info getAddInfoWithStudent:self.stu andGym:self.gym];
...
-(void)createUI
{
    ...
    self.datePicker = [[UIDatePicker alloc]initWithFrame:CGRectMake(0, 39, MSW, 177)];
...
-(void)reloadUI
{
    ...
    self.testTimeTF.text = self.info.date;
    
    self.basicTextFields = [NSMutableArray array];
    
    self.otherTextFields = [NSMutableArray array];
    
    UIView *secView = [[UIView alloc]initWithFrame:CGRectMake(0,Height320(88), MSW, Height320(44)*(self.info.basicTypes.count+self.info.otherTypes.count))];
    ...
    for (NSInteger i = 0;i<self.info.basicTypes.count;i++) {
        ...
        if (!self.info.otherTypes.count && i == self.info.basicTypes.count-1) {
    ...
    for (NSInteger i = 0;i<self.info.otherTypes.count;i++) {
        
        QCTextField *textField = [[QCTextField alloc]initWithFrame:CGRectMake(Width320(16), (i+self.info.basicTypes.count)*Height320(44), MSW-Width320(32), Height320(44))];
        
        MeasureType *type = self.info.otherTypes[i];
        ...
        if (i == self.info.otherTypes.count-1) {
...
-(void)reloadPhotoView
{
    ...
    for (NSInteger i = 0; i<=self.info.photos.count; i++) {
        ...
        if (i<self.info.photos.count) {
            
            NSURL *url = [NSURL URLWithString:self.info.photos[i]];
            
            pictureView.imgUrl = url;
...
-(void)deleteTest:(UIButton*)button
{
    ...
    if ((AppGym && [PermissionInfo sharedInfo].permissions.userPermission.editState)||(!AppGym && [[PermissionInfo sharedInfo]getPermissionStateWithGyms:self.info.measure.student.gyms andPermission:[PermissionInfo sharedInfo].permissions.userPermission andType:PermissionTypeEdit])) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [self uploadImgae:[UIImage fixOrientation:[UIImage imageWithData:UIImageJPEGRepresentation([info objectForKey:UIImagePickerControllerOriginalImage], 0.5)]] withIndex:self.currentIndex];
...
-(void)uploadImgae:(UIImage *)image withIndex:(NSInteger)index
{
    ...
    if (index >= self.info.photos.count) {
        
        [self.info.photos addObject:@""];
        
    }else
    {
        
        [self.info.photos replaceObjectAtIndex:index withObject:@""];
    ...
    uy.successBlocker = ^(NSURLResponse *response,id data){
        ...
        [self.info.photos replaceObjectAtIndex:index withObject:[NSString stringWithFormat:@"http://zoneke-img.b0.upaiyun.com%@",url]];
    ...
    uy.failBlocker = ^(NSError *error){
        ...
        [self.info.photos removeObjectAtIndex:index];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView.tag == 1) {
        ...
        if (buttonIndex == 1) {
            ...
            [self.info.photos removeObjectAtIndex:alertView.tag-100];
...
-(void)keyboardConfirm:(QCKeyboardView *)keyboadeView
{
    ...
    if (keyboadeView.tag == 101) {
        ...
        self.info.date = self.testTimeTF.text;
...
-(void)naviRightClick
{
    ...
    if (self.isAdd) {
        ...
        [self.info createaInfoWithGym:self.gym result:^(BOOL success, NSString *error) {
            ...
            }else
            {
                ...
                [self changeErrorWithMsg:@"添加失败，请稍后重试"];
    ...
    }else
    {
        ...
        [self.info changeInfoResult:^(BOOL success, NSString *error) {
            ...
            }else
            {
                ...
                [self changeErrorWithMsg:@"修改失败，请稍后重试"];
...
-(void)deleteSuccess
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (UIViewController *vc in self.navigationController.viewControllers) {
            ...
            if ([vc isKindOfClass:[MeasuresListController class]]) {
                
                [((MeasuresListController*)vc) reloadData];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (UIViewController *vc in self.navigationController.viewControllers) {
            ...
            }else
            {
                ...
                if ([vc isKindOfClass:[MeasureDetailController class]]) {
                    
                    ((MeasureDetailController*)vc).info = self.info;
                    
                    [((MeasureDetailController*)vc) reloadData];
