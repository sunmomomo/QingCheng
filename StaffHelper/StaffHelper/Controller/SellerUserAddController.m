...
@interface SellerUserAddController ()<UITableViewDelegate,MOTableViewDatasource,UITextFieldDelegate,UserChooseViewDatasource,SellerUserDeleteCellDelegate,UIAlertViewDelegate>
...
@implementation SellerUserAddController
{
    UIView *_header;
}
...
-(void)createData
{
    ...
    weakTypesYF
    [self.info requestAddDataWithSeller:self.seller andGym:self.gym success:^{
...
-(void)reloadData
{
    ...
    weakTypesYF
    [info requestAddDataWithSeller:self.seller andGym:self.gym success:^{
...
-(void)dealUsers
{
    ...
    }else
    {
        if (self.fiterViewModel.buttonOfLetterFilter.isSelected)
        {
            self.users = self.info.showNoSellerArray;
        }else
        {
            self.users = self.info.showNoSellerTimeArray;
        }
        [self setTableFootviewLabelNum:self.info.studentNoSellerCount];
...
//        BOOL chooseable = YES;
//        for (Seller *tempSeller in stu.sellers) {
//            
...
-(void)naviRightClick
{
    ...
    if (self.chooseArray.count) {
        ...
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"确定将所选会员添加到%@名下？",self.seller.name] message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag == 0) {
        ...
        if (buttonIndex == 1) {
            ...
            [info addUsers:self.chooseArray withSeller:self.seller withGym:self.gym result:^(BOOL success, NSString *error) {
                ...
                if (success) {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:kPostAddNewStudentToSellerIdtifierYF object:nil];
...
- (YFSearchResultAddSellerListVC *)resultVC
{
    if (!_resultVC)
    {
        ...
        _resultVC.seller = self.seller;
        
        _resultVC.sellerAddVC = self;
