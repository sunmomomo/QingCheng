...
@interface SellerBelongUserController ()<UITableViewDelegate,MOTableViewDatasource,UIAlertViewDelegate>
...
@implementation SellerBelongUserController
{
    UIView *_header;
}
...
-(void)createUI
{
    ...
    self.title = self.seller.type == SellerTypeNormal?[NSString stringWithFormat:@"%@的名下会员",self.seller.name]:@"无销售";
    
    self.view.backgroundColor = UIColorFromRGB(0xffffff);
    
    self.rightTitle = self.seller.type == SellerTypeNormal?@"批量修改":@"批量分配";
...
{
    SellerUserAddController *svc = [[SellerUserAddController alloc]init];
    ...
    {
        svc.seller_id = self.seller.sellerStrId;
    }
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
    cell.sellerType = self.seller.type;
    }
    ...
    cell.editing = self.seller.type == SellerTypeNormal;
...
-(void)deleteStudent
{
    ...
    [[[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"确认将选中会员从%@名下移除？",self.seller.name] message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定",nil]show];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 1) {
        ...
        [info deleteUser:stu withSeller:self.seller withGym:self.gym result:^(BOOL success, NSString *error) {
