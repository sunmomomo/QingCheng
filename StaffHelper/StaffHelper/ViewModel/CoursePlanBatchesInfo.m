...
#define DELETEAPI @"/api/staffs/%ld/%@/bulk/delete/"

#define kChangeAPI @"/api/staffs/%ld/%@/%ld/"
...
-(void)requestWithCourse:(Course *)course andBatchId:(NSInteger)batchId
{
    ...
    [MOAFHelp AFGetHost:ROOT bindPath:[NSString stringWithFormat:API,StaffId,(long)batchId,course.type == CourseTypeGroup?@"schedules":@"timetables"] param:para.data success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
...
-(void)createDataWithArray:(NSArray *)array
{
    ...
    NSSortDescriptor *descriptor1 = [NSSortDescriptor sortDescriptorWithKey:@"date" ascending:YES];
    
    NSSortDescriptor *descriptor2 = [NSSortDescriptor sortDescriptorWithKey:@"startTime" ascending:YES];
    
    self.batches = [[self.batches sortedArrayUsingDescriptors:[NSArray arrayWithObjects:descriptor1,descriptor2, nil]]mutableCopy];
...
-(void)dealArray
{
    ...
    for (NSString *month in _monthArray) {
        ...
        for (CoursePlan *plan in self.batches) {
...
-(void)changeWithPlan:(CoursePlan *)plan andCourse:(Course *)course
{
    ...
    [MOAFHelp AFPutHost:ROOT bindPath:[NSString stringWithFormat:kChangeAPI,StaffId,course.type == CourseTypeGroup?@"schedules":@"timetables",(long)plan.planId] putParam:para.data success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
...
-(void)deleteWithPara:(Parameters *)para andCourse:(Course *)course
{
    ...
    [MOAFHelp AFPostHost:ROOT bindPath:[NSString stringWithFormat:DELETEAPI,StaffId,course.type == CourseTypeGroup?@"schedules":@"timetables"] postParam:para.data success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
