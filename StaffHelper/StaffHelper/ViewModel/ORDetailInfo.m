...
#define API @"/api/%@/staffs/%ld/reports/%@/"

#define ExportAPI @"/api/staffs/%ld/excel/tasks/"
...
-(void)requestWithFilter:(ReportFilter *)filter andGym:(Gym *)gym result:(void (^)(BOOL, NSString *))result
{
    ...
    NSString *api = [NSString stringWithFormat:API,filter.infoType == ReportInfoTypeSchedule?@"v3":@"v2",StaffId,filter.infoType == ReportInfoTypeSell?@"sells":@"schedules"];
    
    if (filter.infoType == ReportInfoTypeCheckin) {
        
        api = [NSString stringWithFormat:@"/api/staffs/%ld/reports/checkin/",StaffId];
...
-(void)createFormData
{
    ...
    if (_filter.infoType == ReportInfoTypeSchedule) {
        ...
        allCourse.courseType = CourseTypeNone;
...
+(instancetype)exportExcelWithFilter:(ReportFilter *)filter gym:(Gym *)gym title:(NSString *)title email:(NSString *)email result:(void (^)(BOOL, NSString *))result
{
    
    ORDetailInfo *info = [[ORDetailInfo alloc]init];
    ...
    [para setParameter:title forKey:@"title"];
    
    [para setParameter:email forKey:@"to_email"];
    
    NSMutableDictionary *query = [NSMutableDictionary dictionary];
    
    [query setValue:filter.startDate forKey:@"start"];
    
    [query setValue:filter.endDate forKey:@"end"];
    
    if (filter.allGroup) {
        
        [query setValue:@"all_public" forKey:@"course_extra"];
        
    }else if (filter.allPrivate){
        
        [para setValue:@"all_private" forKey:@"course_extra"];
        
    }
    
    if (filter.allPrepaidCardKind) {
        
        [query setValue:@"all_value" forKey:@"cards_extra"];
        
    }else if (filter.allCountCardKind){
        
        [query setValue:@"all_times" forKey:@"cards_extra"];
        
    }else if (filter.allTimeCardKind){
        
        [query setValue:@"all_time" forKey:@"cards_extra"];
        
    }
    
    if (filter.course.courseId) {
        
        [query setValue:[NSNumber numberWithInteger:filter.course.courseId] forKey:@"course_id"];
        
    }
    
    if (filter.coach.coachId) {
        
        [query setValue:[NSNumber numberWithInteger:filter.coach.coachId] forKey:@"teacher_id"];
        
    }
    
    if (filter.seller.sellerId) {
        
        [query setValue:[NSNumber numberWithInteger:filter.seller.sellerId] forKey:@"seller_id"];
        
    }
    
    if (filter.cardKind.cardKindId) {
        
        [query setValue:[NSNumber numberWithInteger:filter.cardKind.cardKindId] forKey:@"card__card_tpl_id"];
        
    }
    
    if (filter.student.stuId) {
        
        [query setValue:[NSNumber numberWithInteger:filter.student.stuId] forKey:@"user_id"];
        
    }
    
    if (filter.tradeType) {
        
        [query setValue:[NSNumber numberWithInteger:filter.tradeType] forKey:@"type"];
        
    }
    
    if (filter.payWay) {
        
        [query setValue:[NSNumber numberWithInteger:filter.payWay] forKey:@"charge_type"];
        
    }
    
    if (filter.checkinType) {
        
        [query setValue:[NSNumber numberWithInteger:filter.checkinType-1] forKey:@"status"];
        
    }
    
    [para setParameter:query forKey:@"query"];
    
    [para setParameter:filter.infoType == ReportInfoTypeSchedule?@"v2_course_report":filter.infoType == ReportInfoTypeSell?@"v2_sell_report":@"checkin_report" forKey:@"action"];
    
    [MOAFHelp AFPostHost:ROOT bindPath:[NSString stringWithFormat:ExportAPI,StaffId] postParam:para.data success:^(AFHTTPSessionManager *operation, NSDictionary *responseDic) {
